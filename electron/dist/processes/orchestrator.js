...
exports.OrchestratorProcess = void 0;
 ...
 */
class OrchestratorProcess extends manager_js_1.ProcessManager {
    superbotHome;
    superbotName;
    sessionId = null;
    isRestart = false;
    ...
    start() {
        ...
        logger_js_1.logger.info('orchestrator', 'Starting');
    ...
    stop() {
        ...
        logger_js_1.logger.info('orchestrator', 'Stopping');
        return super.stop();
    }
    /**
    ...
    async restart() {
        ...
        await super.stop();
        this.isRestart = true;
        logger_js_1.logger.info('orchestrator', 'Restarting (resume)');
        this.start();
    }
    // ── Subclass hook ────────────────────────────────────────────────────
    /**
    ...
    getSpawnOptions() {
        ...
        return {
            ...
            args,
            env: {
                PATH: (0, resolve_node_js_1.getEnrichedPath)(),
     ...
     */
    assemblePrompt() {
        const templatePath = path.join(this.superbotHome, 'templates', 'orchestrator-system-prompt-override.md');
     ...
     */
    updateTeamConfig() {
        const configPath = path.join(this.superbotHome, '.claude', 'teams', this.superbotName, 'config.json');
        try {
     ...
     */
    purgeTeamLeadInbox() {
        const inboxPath = path.join(this.superbotHome, '.claude', 'teams', this.superbotName, 'inboxes', 'team-lead.json');
        try {
    ...
    onChildExit(code, _signal) {
        ...
        logger_js_1.logger.error('watchdog', `Orchestrator crashed (exit code ${code}). ` +
            `Restart attempt ${attempt}/${MAX_CRASH_RESTARTS}`);
        if (attempt >= MAX_CRASH_RESTARTS) {
            logger_js_1.logger.error('watchdog', 'Max restart attempts reached within 60s. Giving up.');
            this.crashRecoveryEnabled = false;
        ...
        logger_js_1.logger.info('watchdog', `Scheduling auto-restart in ${CRASH_RESTART_DELAY_MS / 1000}s...`);
        this.crashRestartTimer = setTimeout(() => {
            if (!this.crashRecoveryEnabled)
                return;
            logger_js_1.logger.info('watchdog', 'Auto-restarting orchestrator after crash');
            this.isRestart = true;
            this.start();
...
exports.OrchestratorProcess = OrchestratorProcess;
