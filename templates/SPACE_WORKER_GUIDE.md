...
When a task involves creating a new Claude Code skill or plugin, **always save to the drafts folder first**:

- **Draft location**: `~/.superbot2/skill-creator/drafts/<skill-name>/`
- **Active/installed location**: `~/.superbot2/skills/<skill-name>/`
...
Create an escalation (written to `/Users/gkkirsch/.superbot2/escalations/untriaged/`) when you hit:
...
```bash
bash /Users/gkkirsch/.superbot2/scripts/create-escalation.sh <type> <space> <project> "<question>" \
...
- `--suggested-auto-rule "rule text"` - **REQUIRED for `decision` and `question` types**. Suggest a plain English rule that could auto-resolve similar future escalations. The script will error if this flag is missing for these types.

After creating an escalation, move to the next unblocked task. Do not stop working.

### Consuming Resolved Escalations

When you start working on a project that had blocking escalations, check `/Users/gkkirsch/.superbot2/escalations/resolved/` for escalations matching your space/project. For each resolved escalation that was blocking your work:
...
## Reading Knowledge Files (IMPORTANT — Context Budget)

**Do NOT read all knowledge files at startup.** Large spaces have 50+ files totaling 200K+ tokens. Reading everything fills your context window and causes API stalls/timeouts.

### Startup reading order

1. `knowledge/INDEX.md` — if it exists, read this first. It summarizes every file so you know what's available without loading everything.
2. `knowledge/conventions.md` — coding standards for this space
3. `knowledge/decisions.md` — past decisions and rationale
4. `knowledge/patterns.md` — established code patterns

### On-demand reading

Read other knowledge files **only when a specific task needs that domain knowledge**. For example, if your task involves Instagram automation, read `knowledge/instagram-*.md`. If it involves email outreach, read `knowledge/email-templates.md`.

### What to NEVER read at startup

- Binary files: PNG, JPG, SVG, screenshots — only read images when a task requires visual inspection
- Files in subdirectories (e.g., `knowledge/screenshots/`) — these are reference archives
- Files > 20KB unless directly relevant to your assigned task
...
When you create or significantly update a knowledge file, update `knowledge/INDEX.md` with a 1-line summary. This helps future workers find relevant files without reading everything. Format:

```markdown
# Knowledge Index
- **conventions.md** — coding standards, stack choices, naming conventions
- **decisions.md** — past architectural and product decisions with rationale
- **patterns.md** — established code patterns and examples
- **stripe-api.md** — Stripe API integration notes, webhook handling, error codes
```
...
6. **Reported to orchestrator** - send a message to "team-lead" with:
   ...
   - **User preferences detected** - if during your work you observed the user explicitly state a preference (in chat, escalation resolutions, or direct instructions), include it prefixed with `[USER_PREF]:` so the orchestrator can update USER.md. Example: `[USER_PREF]: User prefers PostgreSQL over SQLite for all new projects.` Only report explicit user statements, not inferences.
...
## System File Sync — REQUIRED for meta workers

If you are working in the `meta` space and modify any of these files, you MUST sync the change to the other location before finishing:
...
2. If only you changed one side, copy your changes across
3. If both sides differ, merge manually — never blindly overwrite
4. Commit the dev repo: `git add <file> && git commit -m "..."`

Runtime-only files (do NOT sync): `escalations/`, `sessions/`, `todos.json`, `config.json`, `spaces/`, `knowledge/`, `MEMORY.md`, `USER.md`

## Web Application Rules (MANDATORY)

If you are building or modifying a web application, these rules apply without exception:

### 1. Always QA using superbot-browser

Before marking any UI task as complete, you MUST visually verify it in the browser using the `superbot-browser` skill. Do not rely on TypeScript passing or build succeeding alone — open the browser and look at it.
...
- Take a screenshot and inspect the UI
- Interact with the feature you just built (click buttons, fill forms, etc.)
- Confirm it matches the acceptance criteria visually

If the browser shows something broken, fix it before marking the task done.

### 2. Always configure dev server in space.json

When setting up a new web app space or adding a dev server to an existing one, update the space's `space.json` to include the dev server config:
...
Space JSON location: `/Users/gkkirsch/.superbot2/spaces/<slug>/space.json`
...
- Never modify files outside `/Users/gkkirsch/.superbot2/spaces/<your-space>/`.
- Never delete task files. Mark them completed.
- Never modify `/Users/gkkirsch/.superbot2/knowledge/`. Write to your space's knowledge/ instead.
