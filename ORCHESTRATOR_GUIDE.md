...
- Pending and in_progress task subjects (so you know *what* needs doing)
- Escalation details (question, type, priority)
- Use `--compact` flag for counts only (no task details)
...
Do NOT read full plan.md, space knowledge/, OVERVIEW.md, docs/, or anything in the space's `app/` directory. That's the space worker's job.
...
**You (the orchestrator) are the only agent that writes to USER.md.** Space workers never write to USER.md directly.

- When a worker completion message contains `[USER_PREF]:` entries, check if the preference is new and not already captured in USER.md
- Only record explicit user statements — never add inferences, assumptions, or patterns you noticed
- Add new preferences to the appropriate section of USER.md (Working Style, Technical Preferences, etc.)
- If a preference contradicts an existing entry, update the existing entry rather than adding a duplicate
...
**The team config is NOT reliable.** Workers that crash or finish without shutting down remain as zombie processes. Always use the process list as the authoritative source:

```bash
ps aux | grep "claude" | grep "agent-id" | grep -v grep | awk '{for(i=1;i<=NF;i++) if($i=="--agent-id") print $(i+1)}' | sort
```

**At every heartbeat, run this check:**
1. Get the real running worker list from `ps aux` above
2. Cross-check against `portfolio-status.sh` — any worker whose project is 100% done is stale
3. Send `shutdown_request` to all stale workers immediately

**Stale = any of these:**
- Project is 100% complete but worker still running
- Worker with `-2`/`-3` suffix when the original is still alive
- `*-planner` worker after the plan was created and tasks exist
- Any worker running 60+ minutes with no completion message
- Worker names not recognized from the current orchestration cycle

**Never** let workers linger past project completion. Check ps, not just the team config.

### Long-Running Worker Check-ins

For workers that have been running 30+ minutes without sending a message, **check in proactively**. Long-running workers can get stuck, hit errors silently, or go into loops.

```
SendMessage to <worker-name>:
  "Checking in — you've been running for a while. What's your current status?
   What task are you on, and are there any blockers?"
```

**Check-in triggers:**
- 30 minutes without a message → send a check-in
- 60 minutes without a message → stronger nudge, ask for immediate status
- 90 minutes without a message → consider killing and re-spawning if no response

At each heartbeat, note which workers have been running a long time and haven't reported in. Don't assume silence means progress.
...
5. Clean up stale data that causes noisy heartbeats:
...
The dashboard also has a restart button that does the same thing (POST `/api/orchestrator/restart`).

## Rules

- **Stay light** - Don't read full space context. That's the space worker's job.
- **Never read task files** - Never read, glob, or grep task JSON files in `plans/*/tasks/`. Use `portfolio-status.sh` instead — it shows pending task subjects and escalation details.
