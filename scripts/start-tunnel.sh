...
# Generate a rotating access token for tunnel security
ACCESS_TOKEN=$(uuidgen)
echo "Access token: $ACCESS_TOKEN"

# Save URL and access token to config.json
if [ -f "$CONFIG" ]; then
    ...
    node -e "
        ...
        config.telegram.accessToken = '$ACCESS_TOKEN';
        fs.writeFileSync('$CONFIG', JSON.stringify(config, null, 2));
    "
    echo "Saved webAppUrl and accessToken to config.json"

    # Trigger Vite restart to pick up new allowedHosts
    touch "$(dirname "$0")/../dashboard-ui/vite.config.ts"
    echo "Triggered Vite restart for new tunnel hostname"

    # Build tokenized URL for Telegram menu button
    TOKENIZED_URL="${TUNNEL_URL}?token=${ACCESS_TOKEN}"
    ...
    if [ -n "$BOT_TOKEN" ]; then
        RESULT=$(curl -s -X POST "https://api.telegram.org/bot${BOT_TOKEN}/setChatMenuButton" \
            ...
            -d "{\"menu_button\": {\"type\": \"web_app\", \"text\": \"Dashboard\", \"web_app\": {\"url\": \"$TOKENIZED_URL\"}}}")

        if echo "$RESULT" | grep -q '"ok":true'; then
            echo "Updated Telegram menu button to $TOKENIZED_URL"
        else
